...
const { Initial, Glide, Nucleus, Coda, Dialect, Tone, Format } = ConversionConstants;
...
function tokenize(upperText) {
    ...
    const specialPrefix = CommonUtil.longestMatch(SpecialPrefixes, tail);
    const glottalGlideMark = CommonUtil.longestMatch(GlottalGlideMarks, tail);
    ...
    if (specialPrefix) {
        switch (specialPrefix) {
            ...
            case 'IO': {
                ...
                const nonGlideCodaMark = CommonUtil.longestMatch(NonGlideCodaMarks, tempTail);
                const toneMark = CommonUtil.longestMatch(ToneMarks, tempTail);
        ...
        const codaMark = CommonUtil.longestMatch(CodaMarks, tempTail);
        const toneMark = CommonUtil.longestMatch(ToneMarks, tempTail);
        ...
        initialMark = CommonUtil.longestMatch(InitialMarks, tail);
        tail = tail.slice(initialMark.length);

        glideMark = CommonUtil.longestMatch(GlideMarks, tail);
        tail = tail.slice(glideMark.length);
    }

    if (!normalNucleusMark) {
        specialNucleusMark = CommonUtil.longestMatch(SpecialNucleusMarks, tail);
        tail = tail.slice(specialNucleusMark.length);

        if (!specialNucleusMark) {
            normalNucleusMark = CommonUtil.longestMatch(NormalNucleusMarks, tail);
            tail = tail.slice(normalNucleusMark.length);
        }
    }

    const codaMark = CommonUtil.longestMatch(CodaMarks, tail);
    tail = tail.slice(codaMark.length);

    const toneMark = CommonUtil.longestMatch(ToneMarks, tail);
    tail = tail.slice(toneMark.length);

    const ptLongVowelMark = CommonUtil.longestMatch(PTLongVowelMark, tail);
...
function getRhyme(neutralized, options) {
    ...
    if (!coda) {
        if (surfaceNucleus === Nucleus.O) {
            return CommonUtil.isGuiliuTone(tone) ? 'O' : 'OA';
        }
    }
    ...
    if (coda === Coda.P || coda === Coda.T || coda === Coda.K) {
        codaMark = CommonUtil.isHighTone(tone) ? CodaToMark[coda][0] : CodaToMark[coda][1];
