...
const { Initial, Glide, Nucleus, Coda, Tone, ToneCategory, Format } = ConversionConstants;
...
function tokenize(upperText) {
    ...
    const initialMark = CommonUtil.longestMatch(InitialMarks, tail);
    tail = tail.slice(initialMark.length);

    let glideMark = CommonUtil.longestMatch(GlideMarks, tail);
    if (glideMark === 'OA') {
        glideMark = 'O';
    }
    tail = tail.slice(glideMark.length);

    const nucleusMark = CommonUtil.longestMatch(NucleusMarks, tail);
    tail = tail.slice(nucleusMark.length);

    const codaMark = CommonUtil.longestMatch(CodaMarks, tail);
    tail = tail.slice(codaMark.length);

    const wordToneMark = CommonUtil.longestMatch(WordToneMarks, tail);
...
function getInitial(neutralized) {
    ...
    const allPhonations = CommonUtil.getPhonations(initial);
    ...
    return CommonUtil.isHighTone(tone) ? candidates[0] : candidates[1];
}
...
function getRhyme(neutralized) {
    ...
    if (!coda) {
        if (nucleus === Nucleus.O && CommonUtil.isGuiliuTone(tone)) {
            return 'O';
        }
...
function writeToneMark(toneless, tone) {
    ...
    if (CommonUtil.isTaiTone(tone)) {
        let toneMarkIndex;
        const toneCategory = CommonUtil.getToneCategory(tone);
